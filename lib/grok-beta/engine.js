...
const { checkPickAtHardRock } = require("./hardrock");
...
function evaluateGame(game, gameOdds, predPrice) {
  ...
  const tryMarket = (marketKey) => {
    ...
    for (const q of retail) {
      ...
      const hr = checkPickAtHardRock(draft, gameOdds);
      if (!hr.available) {
        rejections.push({
          matchup: `${awayTeam} @ ${homeTeam}`, market: marketKey, side: q.side,
          reason: hr.errors[0] || "not placeable at Hard Rock Bet",
        });
        continue;
      }
      candidates.push({
        ...
        venue: game.venue || "",
        restHome: means.restH,
        restAway: means.restA,
        projHome: proj.lh != null ? round4(proj.lh) : null,
        projAway: proj.la != null ? round4(proj.la) : null,
        injuryNamesHome: (game.homeInjuries || []).filter(i => parseInjuryStatus(i.status, i.detail) > 0).slice(0, 3).map(i => `${i.name} (${i.status})`),
        injuryNamesAway: (game.awayInjuries || []).filter(i => parseInjuryStatus(i.status, i.detail) > 0).slice(0, 3).map(i => `${i.name} (${i.status})`),
        hardRock: { available: true, odds: hr.hrOdds, point: hr.hrPoint, warnings: hr.warnings, notes: hr.notes },
