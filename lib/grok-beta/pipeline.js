...
async function runGrokBeta(opts = {}) {
  ...
  if (!picks.length) {
    if (remaining.length === 0 && skipped.started > 0) {
      ...
      noPlays = `Grok Beta scanned ${remaining.length} unstarted game(s) across ${uniqueSports.join(", ") || "today's feeds"}. Nothing cleared the 2.2% EV / sharp-hold floor.`;
    }
    edgeSummary = noPlays;
  } else {
    edgeSummary = `Grok Beta v0.2 — ${picks.length} straight${picks.length === 1 ? "" : "s"} (${totalUnits.toFixed(2)}u)${parlay ? ` + ${parlay.legs.length}-leg parlay ${parlay.combinedOdds}` : ""}. Sharp Shin fair vs best retail, independent ESPN projection. Not Alpha.`;
  }
  ...
  const published = {
    ...
    insights: picks.length
      ? `v0.2 sharp process: Shin-devig Pinnacle/Circa/Bookmaker when present, line-shop US retail, blend independent ESPN projection. 3-straight card + optimized parlay. Agents reject/cut only.`
      : "",
    sportsScanned: uniqueSports,
