...
const { writeCardCopy } = require("./narrate");
...
async function runGrokBeta(opts = {}) {
  ...
  // ── ON HOLD 2026-08-25 (Ben) — Grok Build / Live Edge paused. Absolute hard stop BEFORE
  // any external API call (Odds API, xAI, Anthropic, OpenAI) and before any blob write, so
  // NO invocation path — cron (already disabled in netlify.toml), the dashboard "Run Live
  // Edge" button, run-picks-be, or a direct HTTP call — can drain usage. To resume, set this
  // to false (and re-enable the grok-beta-daily schedule in netlify.toml).
  const GROK_BETA_ON_HOLD = true;
  if (GROK_BETA_ON_HOLD) {
    console.log("[grok-beta] ON HOLD (2026-08-25) — run skipped, zero API calls, zero writes. Re-enable: set GROK_BETA_ON_HOLD=false.");
    return emptyPublished(dateISO, { noPlays: "Live Edge is paused.", blocked: "ON_HOLD" });
  }
  ...
  const selected = selectAndCap(allCandidates);
  const capped = selected.picks || [];
  const uniqueSports = [...new Set((sportsScanned || []).filter(Boolean))];
  const candidateDoc = {
    date: dateISO,
    version,
  ...
  // Agents may reject or cut stake only. If they dump a leg, promote the next
  // unique-game engine candidate so a noisy LLM cannot blank a +EV card.
  const kept = (validated.picks || []).slice();
  const rejectedKeys = new Set((validated.rejections || []).map(r => `${r.matchup}||${r.pick}`));
  const usedGames = new Set(kept.map(p => p.matchup));
  if (kept.length < MAX_PICKS) {
    for (const c of allCandidates.sort((a, b) => b.ev - a.ev)) {
      if (kept.length >= MAX_PICKS) break;
      if (usedGames.has(c.matchup)) continue;
      if (rejectedKeys.has(`${c.matchup}||${c.pick}`)) continue;
      kept.push(c);
      usedGames.add(c.matchup);
    }
  }
  const picks = kept.slice(0, MAX_PICKS);
  const { parlay } = selectAndCap(picks);
  const totalUnits = picks.reduce((s, p) => s + (parseFloat(p.units) || 0), 0) + (parlay ? parlay.units : 0);
  ...
  if (!picks.length) {
    if (remaining.length === 0 && skipped.started > 0) {
      ...
      noPlays = `Grok Beta scanned ${remaining.length} unstarted game(s) across ${uniqueSports.join(", ") || "today's feeds"}. Nothing cleared the 1.8% EV / sharp-hold floor.`;
    }
    edgeSummary = noPlays;
  } else {
    const copy = await writeCardCopy(picks, { dateFormatted: formatDateLong(dateISO), parlay });
