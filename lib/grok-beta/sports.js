...
const POINTS_BY_LABEL = {
  ...
  NFL_PRESEASON: { mu: 17.4, hfa: 1.0, restCoef: 0.4, sigmaM: 12.5, sigmaT: 9.2, injStarter: 1.2, injBench: 0.4 },
...
const SHARP_BOOKS = new Set([
  "pinnacle", "circasports", "bookmaker", "betonlineag", "lowvig",
]);

const PYTH_EXP = {
  MLB: 1.83, NBA: 14, WNBA: 14, NCAAB: 11.5, NFL: 2.37, NCAAF: 2.37,
  NHL: 2.0, MLS: 1.35, DEFAULT_SOCCER: 1.35,
};

function isSharp(key) { return SHARP_BOOKS.has(String(key || "").toLowerCase()); }
function isRetail(key) { return US_RETAIL.has(String(key || "").toLowerCase()); }

function isNflPreseason(game) {
  const key = String(game && game.sportKey || "");
  if (key.includes("preseason")) return true;
  if ((game && game.label) !== "NFL") return false;
  const t = Date.parse(game.commenceTime || "");
  if (!Number.isFinite(t)) return false;
  const m = Number(new Date(new Date(t).toLocaleString("en-US", { timeZone: "America/New_York" })).getMonth()) + 1;
  return m === 8; // August = NFL preseason
}

function projWeight(game) {
  if ((game && game.family) === "fight") return 0.15;
  if (isNflPreseason(game)) return 0.12;
  const w = {
    MLB: 0.50, NBA: 0.45, WNBA: 0.45, NHL: 0.40, MLS: 0.40,
    EPL: 0.40, "La Liga": 0.40, "Serie A": 0.40, Bundesliga: 0.40, "Ligue 1": 0.40,
    UCL: 0.40, UEL: 0.40, NFL: 0.40, NCAAF: 0.30, NCAAB: 0.30, UFC: 0.15,
  };
  return w[game && game.label] != null ? w[game.label] : 0.35;
}
...
function familyParams(label, family, game) {
  if (family === "runs") return { family, ...FAMILY.runs };
  if (family === "points") {
    const key = (label === "NFL" && game && isNflPreseason(game)) ? "NFL_PRESEASON" : label;
    return { family, ...FAMILY.points, ...(POINTS_BY_LABEL[key] || POINTS_BY_LABEL[label] || POINTS_BY_LABEL.NBA) };
  }
  if (family === "goals") {
...
module.exports = {
  ODDS_SPORTS, US_RETAIL, SHARP_BOOKS, PYTH_EXP,
  sportByKey, familyParams, isPitcherSensitive,
  isSharp, isRetail, isNflPreseason, projWeight,
};
