...
async function validateCandidates(candidates) {
  ...
  for (const c of candidates) {
    ...
    const hard = reasons.some(r => /scratch|postpon|already started|no starter|fighter out/i.test(r));
    if (rejectVotes >= 3 && hard) {
      rejections.push({ matchup: c.matchup, pick: c.pick, reason: reasons.join(" | ") || "3 agents rejected" });
      continue;
    }
    if (rejectVotes >= 1) {
      minUnits = Math.max(0.25, Math.round(c.units * 2) / 4);
      reasons.push("agent caution — stake cut, engine p_model/EV kept");
    }
    const { letterForUnits } = require("./engine");
    const next = {
      ...c,
      units: minUnits,
      rating: letterForUnits(minUnits),
      confidence: minUnits >= 1 ? "a" : minUnits >= 0.75 ? "aminus" : minUnits >= 0.5 ? "bplus" : "b",
    };
