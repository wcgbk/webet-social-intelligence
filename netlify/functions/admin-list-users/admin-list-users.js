// admin-list-users — list wbai-users registrations (admin auth)
...
async function getUsersStore() {
  ...
  // Match auth-sms-verify / auth-x-callback credentials (these successfully write user_* keys)
  if (token) {
    return getStore({ name: 'wbai-users', siteID: SITE_ID, token });
  }
