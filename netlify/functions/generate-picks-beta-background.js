// generate-picks-beta-background.js
// v10.1 — Deterministic Edge Architecture (BETA)
...
// Background function (15min timeout). Stores to "edge-picks-beta" Netlify Blob store.
...
// NBA: 50.0% accuracy, -4.5% ROI (proven drag) → 0.85x penalty
// MLB: season starting, insufficient data → 1.0x
// CLV feedback loop can further adjust these at runtime
const SPORT_KELLY_MULT = { NHL: 1.30, NBA: 0.85 };
...
// ── Sport-specific cover probability caps (from industry calibration) ──
// Sharp models top out at ~56-58% cover rate over a season. These caps reflect
// realistic ceilings — not theoretical maximums from normalCDF.
const COVER_PROB_CAPS = {
  // v10.1: Aligned with production caps — old beta caps (0.64-0.66) were compressing
  // strong edges into mediocre ones, destroying grade differentiation.
  "NHL_Puck Line": 0.60,
  "NHL_Total": 0.75,
  "NHL_Moneyline": 0.72,
  "NBA_Spread": 0.72,
  ...
  "MLB_Total": 0.70,
  "MLB_Moneyline": 0.68,
  // Soccer: draw possibility compresses win prob ceilings
...
function getCalibratedCoverProb(rawProb, sport, market, odds, pmSignal) {
  ...
  const cap = COVER_PROB_CAPS[key] || 0.75; // default max 75% (matches production)
  let calibrated = Math.min(rawProb, cap);

  // Caps alone handle overconfidence. Market blend was removed because
  // double compression (cap + blend) over-dampened edges by 5-9 points,
  // clustering all picks in a narrow probability band and destroying
  // grade differentiation. The logarithmic edge discount in applyEdgeDiscount()
  // already penalizes large market disagreements upstream.
...
function computeKelly(coverProb, odds, drawdownActive) {
  ...
  // Discrete Kelly sizing: fraction × 10 bankroll units, round to 0.5u steps
  // Matches production rounding for consistency
...
async function fetchCLVFeedback() {
  ...
  try {
    const indexResp = await fetch(
      `https://api.netlify.com/api/v1/blobs/${SITE_ID}/edge-picks/picks-dates`,
      { headers: { "Authorization": `Bearer ${token}` } }
    );
    if (!indexResp.ok) return {};
    const dateIndex = await indexResp.json();
    const allDates = (dateIndex.dates || []).sort().reverse().slice(0, 14);

    const clvEntries = [];
    for (const d of allDates) {
      try {
        const resp = await fetch(
          `https://api.netlify.com/api/v1/blobs/${SITE_ID}/edge-picks/clv-${d}`,
          { headers: { "Authorization": `Bearer ${token}` } }
        );
        if (resp.ok) {
          const data = await resp.json();
    ...
    for (const game of sportOdds.games) {
      ...
      for (const book of (game.bookmakers || [])) {
        ...
        for (const mkt of (book.markets || [])) {
          if (mkt.key === 'spreads') {
            for (const o of mkt.outcomes) {
              if (o.point !== undefined) {
                ...
                // Add weighted entries for sharp-book-weighted median
                if (o.name === homeTeam) {
          ...
          // ── Alternate lines: store for best-EV comparison later ──
          if (mkt.key === 'alternate_spreads') {
            if (!entry.altSpreads) entry.altSpreads = [];
            for (const o of mkt.outcomes) {
              if (o.point !== undefined && o.price !== undefined) {
                entry.altSpreads.push({ point: o.point, odds: o.price, team: o.name });
              }
            }
          }
          if (mkt.key === 'alternate_totals') {
            ...
            for (const o of mkt.outcomes) {
              if (o.point !== undefined && o.price !== undefined) {
                ...
                if (dir === 'over') {
                  // Find matching under for same point
                  const existing = entry.altTotals.find(t => t.point === o.point);
                  if (existing) { existing.overOdds = o.price; }
                  else { entry.altTotals.push({ point: o.point, overOdds: o.price, underOdds: null }); }
                } else if (dir === 'under') {
                  const existing = entry.altTotals.find(t => t.point === o.point);
                  if (existing) { existing.underOdds = o.price; }
                  else { entry.altTotals.push({ point: o.point, overOdds: null, underOdds: o.price }); }
                }
              }
            }
          }
...
function computeProjection(game, leagueName, leagueConfig, homeRating, awayRating, teamStats, pitcherData) {
  ...
  const homeStreakAdj = getStreakEloAdjustment(homeRating.streak || 0, leagueName);
  const awayStreakAdj = getStreakEloAdjustment(awayRating.streak || 0, leagueName);
  ...
  const homeWinProb = eloExpected(homeEloAdj, awayEloEffective);
...
// Defaults to the +8% conviction bar. The thin-slate fallback (Option B) re-runs
  ...
  for (const league of espnData) {
    ...
    for (const game of league.games) {
      ...
      if (actualTotal) {
        ...
        let totalEdge = applyEdgeDiscount(rawTotalEdge, std);
      ...
      const homeML = !SOCCER_LEAGUES.includes(league.league) ? gameData.homeML : null;
      ...
      if (homeML && awayML) {
        ...
        const ML_MAX_DOG = { NBA: 200, NHL: 250, NCAAB: 200, MLB: 200 };
        ...
        for (const [side, winProb, ml, teamName] of [
          ...
          const implProb = impliedProb(ml);
          const mlEdge = winProb - implProb;
          ...
          if (implProb < 0.25 && winProb > 0.40) continue;
  ...
  for (const c of candidates) {
    ...
    if (c.market === "Spread" || c.market === "Puck Line" || c.market === "Run Line") {
      ...
      for (const alt of relevantAlts) {
        ...
        if (altKelly.ev > c.ev) {
          ...
          adjUnits = Math.max(0.5, Math.min(3.0, adjUnits));
          c.side = `${pickTeam} ${alt.point > 0 ? '+' : ''}${alt.point}`;
          c.odds = alt.odds;
    ...
    if (c.market === "Total") {
      ...
      for (const alt of relevantAlts) {
        ...
        const discountedEdge = applyEdgeDiscount(altTotalEdge, std);
        const altZ = discountedEdge / std;
        const altRawProb = normalCDF(altZ);
...
function buildFinalPicks(candidateTable, claudeSelections, allCandidates, drawdownActive) {
  ...
  // If 2+ picks share a matchup, their outcomes are correlated (e.g., Blues ML + Over 6
  // in the same game — if Blues get blown out 5-1, ML loses and Over may win, but a 1-0
  // game kills both). Reduce the lower-conviction pick by 25% to limit correlated risk.
  const matchupCount = {};
  for (const p of picks) {
    const key = p.matchup;
    if (!matchupCount[key]) matchupCount[key] = [];
    matchupCount[key].push(p);
  }
  for (const [matchup, group] of Object.entries(matchupCount)) {
    if (group.length >= 2) {
      // Sort by units descending — protect the highest-conviction pick, reduce the rest
      group.sort((a, b) => parseFloat(b.units) - parseFloat(a.units));
      for (let i = 1; i < group.length; i++) {
        const oldU = parseFloat(group[i].units);
        const newU = Math.max(0.5, Math.round((oldU * 0.75) * 2) / 2); // 25% reduction, round to 0.5u
        console.log(`[v10-corr] SAME GAME "${matchup}": ${group[i].pick} reduced ${oldU}u → ${newU}u (correlated with ${group[0].pick})`);
        group[i].units = `${newU}u`;
        group[i].rating = unitsToRating(newU);
        group[i].confidence = ratingToConfidence(group[i].rating);
      }
    }
  }

  // (Daily 4.0u cap already enforced in buildFinalPicks above)
  ...
  for (const p of picks) {
    ...
    //    Under 10% EV should never exceed 1.0u
    if (ev < 0.10 && parseFloat(p.units) > 1.0) {
      console.log(`[v10-validate] DOWNSIZE ${p.pick}: → 1.0u (EV ${(ev*100).toFixed(1)}% < 10%)`);
      p.units = "1.0u";
      p.rating = unitsToRating(1.0);
      p.confidence = ratingToConfidence(p.rating);
    }
  ...
  for (let i = 0; i < pool.length - 2; i++) {
    for (let j = i + 1; j < pool.length - 1; j++) {
      for (let k = j + 1; k < pool.length; k++) {
        ...
        // Correlation penalty: same-game legs are correlated, reduce combined prob
        const matchups = trio.map(t => `${t.homeTeam}|${t.awayTeam}`);
        const uniqueMatchups = new Set(matchups).size;
        if (uniqueMatchups < 3) {
          // Same-game correlation: reduce combined prob by 10% per shared game
          const sharedGames = 3 - uniqueMatchups;
          combinedProb *= Math.pow(0.90, sharedGames);
        }
...
exports.handler = async (event) => {
  ...
  if (allCandidates.length === 0) {
    ...
    let leanCandidates = [];
    try {
      leanCandidates = computeEdgeTable(espnData, ratingsData, teamStats, consensusLookup, bankrollCtx.drawdownActive, calibrationData, pitcherData, 0.03);
    ...
    await storePicks(dateISO, {
      date: dateISO, dateFormatted, model: "v10.1-deterministic-edge",
      ...
      summary: { totalPicks: 0, totalStraightBets: 0, totalUnits: "0u", aplusLocks: 0, sportsCovered: [], modelVersion: "v10.1-deterministic-edge" },
  ...
  try {
    ...
    const picksData = {
      date: dateISO,
      dateFormatted,
      model: "v10.1-deterministic-edge",
      picks,
      rejections,
      ...
      summary: {
        ...
        sportsCovered,
        modelVersion: "v10.1-deterministic-edge",
      },
      generatedAt: now.toISOString(),
...
async function buildThinSlatePicks(dateISO, dateFormatted, leanCandidates, now) {
  ...
  const picksData = {
    ...
    summary: { totalPicks: picks.length, totalStraightBets: picks.length, totalUnits: `${totalUnits.toFixed(2)}u`, aplusLocks: 0, sportsCovered: [...new Set(picks.map(p => p.sport))], modelVersion: "v10.1-deterministic-edge" },
...
async function fallbackToTopCandidates(dateISO, dateFormatted, candidateTable, allCandidates, now) {
  ...
  const picksData = {
    ...
    summary: { totalPicks: picks.length, totalStraightBets: picks.length, totalUnits: `${totalUnits.toFixed(1)}u`, aplusLocks: 0, sportsCovered: [...new Set(picks.map(p => p.sport))], modelVersion: "v10.1-deterministic-edge" },
    generatedAt: now.toISOString(), parlayLegs: [], sgps: [],
    fallback: true,
  };
...
// ── Store picks to "edge-picks-beta" blob store ──
async function storePicks(dateISO, picksData) {
  ...
  // Trigger Kalshi auto-execution after picks are stored
  try {
    const siteURL = process.env.URL || 'https://webetsocial.com';
