// get-picks-be.js
// API endpoint: GET /.netlify/functions/get-picks-be
// Returns the latest BettorEdge-enhanced picks JSON from Netlify Blobs.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      const store = getStore("edge-picks-be");
      ...
      if (requestedDate && /^\d{4}-\d{2}-\d{2}$/.test(requestedDate)) {
        ...
        if (!latestDate) {
          return {
            ...
            body: JSON.stringify({
              error: false,
              noPlays: "No BettorEdge picks generated yet. Trigger via /api/run-picks-be.",
              date: null,
              picks: [],
              rejections: [],
      ...
      if (!picksData) {
        return {
          ...
          body: JSON.stringify({
            error: false,
            noPlays: `No BettorEdge picks found for ${dateKey}.`,
            date: dateKey,
            picks: [],
            rejections: [],
            summary: { totalPicks: 0, totalUnits: "0u", aplusLocks: 0, sportsCovered: [] },
      ...
      console.error("[get-picks-be] Blobs SDK error:", blobErr.message);

      const token = process.env.NETLIFY_AUTH_TOKEN;
      const siteId = process.env.SITE_ID || "87d7bcd9-e95a-479c-bc44-6432a2ffc606";

      if (token) {
        const baseUrl = `https://api.netlify.com/api/v1/blobs/${siteId}/edge-picks-be`;
        ...
        if (!dateKey) {
          try {
            ...
            console.error("[get-picks-be] Latest date fetch error:", e.message);
          }
        }
        ...
        if (dateKey) {
          try {
            ...
            console.error("[get-picks-be] Picks fetch error:", e.message);
          }
        }
      }
    ...
    console.error("[get-picks-be] Error:", err.message);
    return {
      statusCode: 500,
      headers: CORS,
      body: JSON.stringify({ error: true, message: "Failed to fetch BettorEdge picks" }),
    };
  }
};
