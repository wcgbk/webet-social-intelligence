// get-picks-beta.js
// API endpoint: GET /.netlify/functions/get-picks-beta
// Returns the latest picks JSON from Netlify Blobs (beta store).
...
exports.handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      const store = getStore("edge-picks-beta");
      ...
      console.error("[get-picks-beta] Blobs SDK error:", blobErr.message);
      console.log("[get-picks-beta] Falling back to Netlify API");

      const token = process.env.NETLIFY_AUTH_TOKEN;
      const siteId = process.env.SITE_ID || "87d7bcd9-e95a-479c-bc44-6432a2ffc606";

      if (token) {
        const baseUrl = `https://api.netlify.com/api/v1/blobs/${siteId}/edge-picks-beta`;
        ...
        if (!dateKey) {
          try {
            ...
            console.error("[get-picks-beta] Latest date fetch error:", e.message);
          }
        }
        ...
        if (dateKey) {
          try {
            ...
            console.error("[get-picks-beta] Picks fetch error:", e.message);
          }
        }
      }
    ...
    console.error("[get-picks-beta] Error:", err.message);
    return {
      statusCode: 500,
      headers: CORS,
