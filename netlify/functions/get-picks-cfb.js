// get-picks-cfb.js
// API endpoint: GET /api/get-picks-cfb
// Returns today's college football card from edge-picks-cfb. Off days (or a missing today
...
function emptyPayload(dateKey, msg) {
  return {
    ...
    noGames: /No College Football Games Scheduled/i.test(msg || NO_GAMES_MSG),
...
exports.handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      if (token) {
        const baseUrl = `https://api.netlify.com/api/v1/blobs/${siteId}/edge-picks-cfb`;
    ...
    return {
      ...
      body: JSON.stringify({ error: true, message: "Failed to fetch CFB picks" }),
    };
  }
};
