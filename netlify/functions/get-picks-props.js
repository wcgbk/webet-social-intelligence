// get-picks-props.js
// API endpoint: GET /.netlify/functions/get-picks-props
// Returns the latest player props picks JSON from Netlify Blobs (props store).
  ...
  try {
    ...
    async function resolveLatestDate() {
      try {
        ...
        if (token) {
          const resp = await fetch(`https://api.netlify.com/api/v1/blobs/${siteId}/edge-picks-props/latest-date`, {
    ...
    async function readPicks(dateKey) {
      ...
      try {
        ...
        const siteId = process.env.SITE_ID || "87d7bcd9-e95a-479c-bc44-6432a2ffc606";
        if (token) {
          const resp = await fetch(`https://api.netlify.com/api/v1/blobs/${siteId}/edge-picks-props/picks-${dateKey}`, {
            headers: { "Authorization": `Bearer ${token}` }
          });
          if (resp.ok) return await resp.json();
        }
      } catch (e) { console.error("[get-picks-props] API read error:", e.message); }
      return null;
    }
    ...
    return {
      ...
      body: JSON.stringify({
        error: false,
        noPlays: "No player props picks generated yet. Check back after 10am EST.",
        date: null,
        picks: [],
        rejections: [],
        summary: { totalPicks: 0, totalUnits: "0u", sportsCovered: [] },
      }),
    };
  } catch (err) {
    console.error("[get-picks-props] Error:", err.message);
    return {
      statusCode: 500,
      headers: CORS,
      body: JSON.stringify({ error: true, message: "Failed to fetch props picks" }),
    };
  }
};
