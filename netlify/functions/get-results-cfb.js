// get-results-cfb.js
// Returns pick history + clean CFB-only KPIs for the /cfb track record.
// Reads ONLY the edge-picks-cfb store (the college football model's separate environment)
// and grades lazily against ESPN CFB finals on read — same architecture as get-results-nfl,
// with a brand-new stat line starting from zero for the CFB model.
...
// School key = the name minus its nickname word ("lsu tigers" → "lsu"). CFB-safe fuzzy key.
...
function teamsMatch(pickTeam, espnTeam, espnAbbr) {
  ...
  // School-prefix match (NOT nickname): "georgia bulldogs" vs "georgia" → true;
  // "lsu tigers" vs "auburn tigers" → false.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // CFB runs game days only — enumerate ONLY dates that actually have stored cards, not
    // every calendar day (a full-gap calendar would fill the table with off-day rows).
    const allDates = [...cfbDates].sort();
    ...
    const gradedByDate = await Promise.all(allDates.map(async (dateISO) => {
      ...
      if (picksData.noGames || /No College Football Games Scheduled/i.test(picksData.noPlays || '')) {
        return null;
      }
      const day = await gradeDay(dateISO, picksData);
      if (!day) {
        return { date: dateISO, dateFormatted: picksData.dateFormatted || dateISO, source: 'cfb', noPlays: true, picks: [], wins: 0, losses: 0, pushes: 0, pending: 0, accuracy: '0', wagered: 0, profit: 0, roi: '--', parlayResult: 'skip', parlayProfit: 0 };
      }
      return day;
    }));
