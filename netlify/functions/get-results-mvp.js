// get-results-mvp.js
// API endpoint: GET /.netlify/functions/get-results-mvp
// Returns historical picks graded against ESPN final scores — reads from "edge-picks-mvp" blob store.
...
function teamsMatch(pickTeam, espnTeam, espnAbbr) {
  ...
  // Abbreviation match: require exact word-token match only
  // "NO" must not match inside "new orleans", "ORL" must not match inside "orleans"
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Check for cached results first (cache for 5 minutes)
    let cachedResults = null;
    try {
      const cacheResp = await fetch(`${storeUrl}/results-cache-mvp`, { headers: authHeaders });
      if (cacheResp.ok) {
    ...
    for (const dateISO of recentDates) {
      ...
      // Check for optimized parlay legs (may include PARLAY ONLY replacements)
    ...
    // Cache the results
    try {
      await fetch(`${storeUrl}/results-cache-mvp`, {
        method: 'PUT',
    ...
    console.error('[get-results-mvp] Error:', err.message);
    return {
      statusCode: 500,
      headers: CORS,
