...
exports.handler = async (event) => {
  ...
  // 2026-06-12: repointed from edge-picks-omega/results-cache (production store — FROZEN since the
  ...
  try {
    const resultsResp = await fetch(
      `https://api.netlify.com/api/v1/blobs/${SITE_ID}/edge-picks-omega/results-omega-cache-v1`,
    ...
    if (allResults.length < 10) {
      const legacyResp = await fetch(
        `https://api.netlify.com/api/v1/blobs/${SITE_ID}/edge-picks-omega/results-cache`,
        { headers: { "Authorization": `Bearer ${token}` } }
      );
      if (legacyResp.ok) {
