// track-clv-cfb.js
// Captures Pinnacle/sharp closing-line value for the college football card (edge-picks-cfb).
// Separate store from alpha/NFL so this can later merge into the unified CLV loop.
// Invoked by trigger-clv after the alpha + NFL passes.
...
const STORE = 'edge-picks-cfb';
const CFB_SPORTS = ['americanfootball_ncaaf'];
...
// School key = name minus the nickname word ("lsu tigers" → "lsu").
...
function findMatchingGame(pick, oddsData) {
  ...
  for (const game of oddsData) {
    const homeSchool = schoolKey(game.home_team);
    const awaySchool = schoolKey(game.away_team);
    const homeIn = teamsMatch(game.home_team, matchup) || (homeSchool.length > 2 && matchup.includes(homeSchool));
    const awayIn = teamsMatch(game.away_team, matchup) || (awaySchool.length > 2 && matchup.includes(awaySchool));
    if (homeIn && awayIn) return game;
    const homePick = teamsMatch(game.home_team, pickText) || (homeSchool.length > 2 && pickText.includes(homeSchool));
    const awayPick = teamsMatch(game.away_team, pickText) || (awaySchool.length > 2 && pickText.includes(awaySchool));
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!picksData || !Array.isArray(picksData.picks) || !picksData.picks.length) {
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ error: false, message: `No CFB picks for ${dateISO}`, date: dateISO, totalTracked: 0, totalPicks: 0 }) };
    }
    ...
    for (const pick of picksData.picks) {
      ...
      const baseRecord = {
        pick: pick.pick, matchup: pick.matchup, sport: 'NCAAF',
      ...
      if (!close || !close.overround) {
        ...
        console.log(`[track-clv-cfb] ${pick.pick}: bet=${pick.odds} close=${close.sideOdds} clv=${rec.clvCents}c`);
      }
    ...
    const payload = {
      ...
      model: picksData.model || 'v1.0-cfb',
    ...
    console.error('[track-clv-cfb]', err.message);
