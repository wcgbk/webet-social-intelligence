 ...
 *
 * Default dry-run. Use --write to persist omega-replay/{runId}/* (never live picks).
...
async function main() {
  ...
  if (!from) {
    console.error('Usage: node scripts/replay-omega-cli.js --from YYYY-MM-DD [--to YYYY-MM-DD] [--dry-run|--write] [--max-days N] [--run-id ID] [--sports MLB,NFL]');
    console.error(`Defaults: dry-run, max-days=${DEFAULT_MAX_DAYS} (hard cap ${HARD_MAX_DAYS})`);
    console.error('Example (last NFL week window): --from 2026-09-15 --to 2026-09-21 --dry-run');
    process.exit(2);
  }
  console.log(JSON.stringify({ from, to, dryRun, maxDays, runId, sports }, null, 2));
