#!/usr/bin/env node
// Offline unit checks for Omega F5-off-card + MLB underdog ML/RL gates.
// Alpha is the restored pre-coupling control (v10.3) — do not assert Omega gates on it.
...
const royalsRlWithWin = { ...royalsRlFlip, homeWinProb: 52 };
const runLineToday = {
...
const runLineMarketFav = { ...runLineToday, coverProb: 0.5415, homeWinProb: 52 };
...
check('Omega skips plus-money F5 ML at candidate build', () => {
  assert.ok(omegaSrc.includes('if (F5_SKIP_ML_UNDERDOG && (data.price || 0) > 0) continue'));
});
check('Omega has RL rejection reason strings', () => {
  assert.ok(omegaSrc.includes('underdog RL coverProb < 0.50'));
  assert.ok(omegaSrc.includes('underdog RL winProb < 0.50'));
});
check('Claude verify-only path on Omega', () => {
  assert.ok(omegaSrc.includes('JS-locked') || omegaSrc.includes('js-lock'));
...
check('lean pad ON on Omega (always 3)', () => {
