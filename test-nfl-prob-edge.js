...
console.log('\nNFL v1.2-nfl-prob-edge');

check('MODEL_VERSION is v1.2-nfl-prob-edge', () => {
  assert.strictEqual(nfl.MODEL_VERSION, 'v1.2-nfl-prob-edge');
});

check('dual-floor + sigmaML config present', () => {
  ...
  assert.strictEqual(cfg.sigmaML, 13.2);
});
...
check('a genuine in-gate dog ML (short price, real edge) still ships', () => {
  const legit = ml({ side: 'Carolina Panthers ML', home: 'Carolina Panthers', away: 'Chicago Bears',
    odds: 130, coverProb: 0.48, noVigPrior: 0.435, consensusSpread: 2.5 }); // probEdge 4.5pp, +130
